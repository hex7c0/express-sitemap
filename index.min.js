...
module.exports = sitemap, Sitemap.prototype.generate = function(app, router, store) {
    ...
    for (var map = Object.create(null), routing = app._router ? app._router.stack : app.stack, i = 0, ii = routing.length; i < ii; ++i) {
        var route = routing[i];
        if (route.route) (route = routing[i].route) && route.methods && (route.methods.get || route.methods._all) && (map[route.path.toString()] = [ "get" ]); else if (route.handle && route.handle.stack && router) {
            for (var handle, j = 0, jj = router.length; j < jj; ++j) if (route.regexp.test(router[j])) {
                handle = router[j];
                break;
            }
            if (handle && ("/" === handle && (handle = ""), route = this.generate4(route.handle, router, !1))) for (j = 0, 
    ...
    return !0 === langFlag && (head = head.substr(0, head.length - 1) + ' xmlns:xhtml="http://www.w3.org/1999/xhtml">'), 
    head + data + "</urlset>";
