...
module.exports = sitemap, Sitemap.prototype.generate = function(app, router, store) {
    ...
    for (var map = Object.create(null), routing = void 0 !== app._router ? app._router.stack : app.stack, i = 0, ii = routing.length; ii > i; i++) {
        var route = routing[i];
        if (route.route) route = routing[i].route, route && route.methods && (route.methods.get || route.methods._all) && (map[route.path] = [ "get" ]); else if (route.handle && route.handle.stack && router) {
            for (var handle, j = 0, jj = router.length; jj > j; j++) if (route.regexp.test(router[j])) {
                handle = router[j];
                break;
            }
            if (handle) {
                ...
                if (route) for (route = Object.keys(route), j = 0, jj = route.length; jj > j; j++) map[handle + route[j]] = [ "get" ];
            }
        }
    }
    ...
    for (var map = Object.create(null), routing = app.routes.get, i = 0, ii = routing.length; ii > i; i++) {
        var route = routing[i];
        route && route.path && (map[route.path] = [ "get" ]);
    }
    ...
    for (var uri in sitemap) {
        ...
        if (!rr || !rr.disallow && !rr.hide) {
            if (data += "<url><loc>" + this.my.url + uri + "</loc>", "object" == typeof rr && (void 0 !== rr.lastmod && (data += "<lastmod>" + rr.lastmod + "</lastmod>"), 
            void 0 !== rr.changefreq && (data += "<changefreq>" + rr.changefreq + "</changefreq>"), 
            void 0 !== rr.priority && (data += "<priority>" + rr.priority + "</priority>"), 
            void 0 !== rr.alternatepages)) {
