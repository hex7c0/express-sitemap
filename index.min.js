...
module.exports = sitemap, Sitemap.prototype.generate = function(app, router, store) {
    ...
    for (var map = Object.create(null), routing = void 0 !== app._router ? app._router.stack : app.stack, i = 0, ii = routing.length; ii > i; i++) {
        var route = routing[i];
        if (void 0 !== route.route) route = routing[i].route, route && route.methods && (route.methods.get || route.methods._all) && (map[route.path] = [ "get" ]); else if (route.handle && route.handle.stack && router) {
            for (var handle, j = 0, jj = router.length; jj > j; j++) route.regexp.test(router[j]) && (handle = router[j]);
            if (handle) {
